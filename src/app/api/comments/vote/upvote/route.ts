...
import { checkIfVotedOnComment, removeCommentDownvote, upvoteComment } from "@/lib/data";
...
export async function POST( req: Request ) {
    try {
        ...
        if (VOTED.downvote === true) { 
            await removeCommentDownvote({ userID: user.id, commentID: COMMENT_ID });
            await redis.decr(`comments:downvotes:${COMMENT_ID}`);
        }

        const upvoted = await upvoteComment({ userID: user.id, commentID: COMMENT_ID });
        await redis.incr(`comments:upvotes:${COMMENT_ID}`);
