...
import { deletePostAsAdmin } from "@/lib/data";
import { legacy_logError } from "@/lib/utils";

export async function DELETE(req: Request, { params }: { params: Promise<{ postID: string, communityID: string }> }) {
    try {
        const { postID: POST_ID, communityID: COMMUNITY_ID } = await params;
        ...
        try {
            const deletedPost = await deletePostAsAdmin({ userID: user.id, postID: POST_ID, communityID: COMMUNITY_ID });
